...
//Scheduler to Backup data 
//Scheduler to Backup data 
function backupDatabase(transporter) {
  const dbConfig = {
    host: "localhost",
    user: "root",
    password: "", // Replace with your database password if applicable
    database: "alumni_db", // Replace with your database name
  };

  const backupFileName = `database_backup_${new Date().toISOString().replace(/:/g, '-')}.sql`;
  const backupFilePath = `./backups/${backupFileName}`;

  const command = `mysqldump --host=${dbConfig.host} --user=${dbConfig.user} ${
    dbConfig.password ? `--password=${dbConfig.password}` : ""
  } --skip-column-statistics ${dbConfig.database} > "${backupFilePath}"`;

  exec(command, (error, stdout, stderr) => {
    if (error) {
      console.error("Error backing up the database:", error.message);
    } else {
